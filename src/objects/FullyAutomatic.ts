...
export default class FullyAutomatic extends Group {
  camera: Camera;
  prevRotationY: number;

  // recoil/hold config - configuración propia para fully (distinta de pistol)
  private baseRot = new Euler(0, Math.PI / 2, 0); // orientado a la derecha
  private basePos = new Vector3(0.45, -0.28, -0.95); // un poco más lejos y bajo que pistol
  private firing: boolean = false; // cooldown flag
  private fireRate = 15; // shots per second (higher than pistol)
  ...
  constructor(camera: Camera, callback?: (weapon: FullyAutomatic) => void) {
    super();
    this.camera = camera;
    ...
    // Escala propia para fully (ligeramente más grande que pistol)
    this.scale.set(0.12, 0.12, 0.12);

    // Load GLTF model - using the fully semi-automatic model
    const loader = new GLTFLoader();
    loader.load(
      "/models/fully semi-utomatic.glb",
      (gltf) => {
      ...
      undefined,
      (error) => {
        console.error("Error loading fully automatic model:", error);
        // Fallback to low poly if model fails to load
        this.createLowPolyFullyAutomatic();
        camera.getWorldPosition(this.prevCameraPos);
        if (callback) {
          callback(this);
        }
  ...
  private createLowPolyFullyAutomatic() {
    ...
    const addEdgesToBox = (parent: Group, width: number, height: number, depth: number, posX: number, posY: number, posZ: number) => {
      ...
      const r = FullyAutomatic.EDGE_THICKNESS;
      const edges = [
        // Bottom edges
    ...
    // Stock (butt) - larger and longer for rifle
    addEdgesToBox(this, 1.5, 4, 1.2, -2, 0, 0);

    // Main body - longer than pistol
    addEdgesToBox(this, 6, 2, 1.5, 2, 1, 0);

    // Barrel - longer barrel for rifle
    addEdgesToBox(this, 2, 1, 1, 7, 1, 0);

    // Magazine - larger magazine
    addEdgesToBox(this, 1.2, 3, 0.8, 1, -1, 0);

    // Trigger guard
    addEdgesToBox(this, 1, 2, 0.4, 0.5, 0.5, 0);
  }
  ...
  public shoot() {
    ...
    // Adjusted recoil for fully automatic (more intense)
    const kickZ = -0.12;
    const tiltX = 0.25;
    const twistZ = 0.08;
    ...
    this.tl = gsap
      ...
      .to(
        this.position,
        { z: this.basePos.z + kickZ, duration: 0.05, ease: "power3.in" },
        0
      )
      .to(
        this.rotation,
      ...
      .to(this.position, { z: this.basePos.z + kickZ * 0.3, duration: 0.04 })
      .to(
        this.rotation,
        {
          x: this.baseRot.x + tiltX * 0.4,
          z: this.baseRot.z + twistZ * 0.4,
          duration: 0.04,
        },
        "<"
      )
      .to(this.position, { z: this.basePos.z, duration: 0.06 })
      .to(
        this.rotation,
        { x: this.baseRot.x, z: this.baseRot.z, duration: 0.06 },
        "<"
      );
